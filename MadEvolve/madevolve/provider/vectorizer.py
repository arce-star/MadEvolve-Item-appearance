...
class Vectorizer:
    ...
    def embed(self, code: str) -> List[float]:
        ...
        NOTE: Patched to use hash-based deterministic dummy embedding
        to avoid OpenAI API dependency. The dummy preserves cache semantics
        (same code → same vector, different code → different vector).
        """
        cache_key = self._compute_cache_key(code)
        if self.config.cache_embeddings and cache_key in self._cache:
            return self._cache[cache_key]

        # Deterministic dummy: hash code → seed NumPy RNG → generate pseudo-embedding
        import hashlib
        h = int(hashlib.sha256(code.encode()).hexdigest()[:8], 16) % (2**31 - 1)
        rng = __import__('numpy').random.RandomState(h)
        embedding = list(rng.randn(self.config.dimensions).astype(float))
        # Normalize to unit length (cosine similarity semantics preserved)
    ...
    def embed_batch(self, codes: List[str]) -> List[List[float]]:
        """Generate embeddings for multiple codes (dummy)."""
